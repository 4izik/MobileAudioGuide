...
<img width="600" src="https://user-images.githubusercontent.com/79922250/177060830-476aa902-e89e-46f6-b631-37d6c7fc3aaf.png">
...
- **excursionDuration** - продолжительность экскурсии. Пишется не только цифра, но и в чем изменяется. Например: "1 day";
- **routeDistance** - длина маршрута экскурсии. Пишется не только цифра, но и в чем изменяется. Например: "6 km";
- **transportType** - способ передвижения по марщруту. Например: "on foot";
...
- **mapScreenCoordinates** - если развернуть этот параметр, в нем есть два поля, отвечающих за георграфиеские координаты точки, которая будет изначально отображаться в центре экрана карты при переходе на него:
...
1. Имя файла должно состоять только из **префикса** (значение поля **filenamePrefix** из **Excursions.plist**) и **цифры**, означающий порядковый номер тура этого фото/аудио/текста. Например: если в поле filenamePrefix из Excursions.plist указано "BestExcursion", то для первой точки маршрута будут соответствовать файлы BestExcursion1.jpg, BestExcursion1.mp3 и BestExcursion.txt.
...
3. Нельзя добавлять дополнительные символы в цифры, например, файл с именем BestExcursion001.jpg будет проигнорировать, так как в нем указана цифра "001" вместо "1".
